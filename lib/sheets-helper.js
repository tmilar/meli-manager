...
class SheetsHelper {
  ...
  async setRowValuesInRowCells(rowValues, rowNumber) {
    ...
    await Promise.map(rowValues, (cellValue, i) => {
      ...
      const setCellValue = Promise.promisify(cells[i].setValue, {context: cells[i]})
  ...
  async appendNewRow(rowValues) {
    const newRowPosition = await this.getNextEmptyRowPosition()
    await this.ensureSheetSpace(newRowPosition)
    await this.setRowValuesInRowCells(rowValues, newRowPosition)
  }

  async updateOrAppendRow(rowValues, idColumn) {
    const idValue = rowValues[idColumn - 1]
    const allIdValues = await this.getAllCellsByColumn({col: idColumn})
    const matchesId = value => value === String(idValue)

    const updateRow = allIdValues && allIdValues.length > 0 && allIdValues.findIndex(matchesId)

    if (updateRow >= 0) {
      console.log(`Updating row ${updateRow} of id ${idValue} `)
      await this.setRowValuesInRowCells(rowValues, updateRow)
      return
    }
