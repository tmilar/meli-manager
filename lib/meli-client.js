  ...
  async getQuestion(id) {
    ...
    const mapQuestionsResponse = ({account, response}) => {
      if(!response || !response.seller_id) {
        // return an empty account object + 404 error here?
        console.error(`No response or no response seller id! ${response && JSON.stringify(response)}`)
        return {account, response}
      }
      const sellerAccount = authenticatedAccounts.find(acc => acc.id === response.seller_id)
