...
class MeliClient {
  ...
  async getOrders({accounts, id} = {}) {
    ...
    let orders = []
    try {
      orders = await Promise.map(authenticatedAccounts, async account => ({
        account,
        response: await this.client.get({resource, qs, account})
          .catch(error => {
            error.message = `Problem with meli orders request for ${account.nickname}. ${error.message} `
            throw error
          })
      }))
    } catch (error) {
  ...
  async getQuestions({accounts, status} = {}) {
    ...
    return await Promise.map(authenticatedAccounts, async account => {
      let response

      try {
        response = await this.client.get({account, resource, qs})
      } catch (error) {
        response = error.error || error || {}
        response.message = `Problem with meli GET questions '${resource}' request for ${account.nickname}. ${response.message}`
  ...
  async getQuestion(id) {
    ...
    try {
      questionResponse = await Promise.map(singleAccountArray, async account => {
        try {
          const response = await this.client.get({account, resource, data})
          return {response}
        } catch (error) {
